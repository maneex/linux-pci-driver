...
| Runtime utilisateur | commencé — `runtime/`, cœur C : `libvelocitor.{a,so}`, énumération par `/sys/class/velocitor`, `open`/`close` sur handle opaque, `ALLOC`/`FREE`/`STAT`/`INFO`. Enveloppe C++ du §10.3 non commencée |
| Tests | couche 1 — `devtools/qtest-probe.sh`, 181 vérifications sans Linux, transport compris : parcours des `notifyid`, fenêtre `VQ_*`, routage du `DOORBELL`, balayage de l'`avail`, purge au reset, `ERR_DROPPED`, et le plan de contrôle de bout en bout — `ALLOC` avec son `dev_offset` au bas du nœud 0, `STAT` et l'asymétrie 112/128 Mio du §3.2, `FREE` puis le même `FREE` refusé en `ERR_CODE = 3`, et le bit 5 d'`ERR_INJECT` qui se désarme entre deux allocations. Le plan de données du §8 : une chaîne de deux descripteurs, `COPY_H2D` puis relecture par la fenêtre glissante — deux chemins disjoints vers les mêmes octets — `COPY_D2H` vers une adresse hôte que l'aller n'a pas touchée, et le refus d'une génération périmée en `ERR_CODE = 9`. Couche 2 — `devtools/guest-dma-test.sh`, aller-retour DMA, contrôle croisé fenêtre / DMA et passe de *fuzzing* rejouable par graine ; `devtools/guest-trace-test.sh`, anneau de trace, avec le contrôle croisé `dropped` / `skipped` entre les deux implémentations. `devtools/guest-ctrl-test.sh` avec `runtime/ctrl-test`, 34 vérifications du plan de contrôle depuis une application : énumération sysfs, ouverture exclusive, l'asymétrie 112/128 Mio du §3.2, la baisse exacte du libre après `ALLOC`, les deux moitiés de la sémantique de `FREE` du §7.2, et le plantage injecté suivi de la reprise du §6.5 avec le `-ESTALE` sur un handle périmé. Couche 3 inexistante |
